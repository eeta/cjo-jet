	...
	function partyBoard(){ 
		...
		xmlhttp.onreadystatechange = function(){
			if(xmlhttp.readyState == 4 && xmlhttp.status == 200){
				...
				var partyListBox = document.getElementById("party_list_box");
				
				var childCount = partyListBox.childNodes.length;
				for(var i = 0 ; i < childCount ; i++){
					partyListBox.childNodes[0].remove();
				}
				...
				for(partyData of obj){
					...
					title_Atag.setAttribute("href","${pageContext.request.contextPath}/party_board/read_party_board_page.do?jet_board_party_no="+partyData.partyBoardVo.jet_board_party_no);
					title_Atag.setAttribute("style","text-decoration:none;color:#000");
					title_Atag.innerText = partyData.partyBoardVo.jet_board_party_title;
					
					var recruitmentBox = document.createElement("td");
					recruitmentBox.setAttribute("class","text-center")
					recruitmentBox.innerText = partyData.partyBoardVo.jet_board_party_fixcount + "/" + partyData.partyBoardVo.jet_board_party_headcount;
					
					var endgatherDateBox = document.createElement("td");
					endgatherDateBox.setAttribute("class","text-center");
					
					var d = new Date(partyData.partyBoardVo.jet_board_party_endgather);
					...
					var d = new Date(partyData.partyBoardVo.jet_board_party_tsday);
					var strDate = d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate(); 
		...
		xmlhttp.open("get","${pageContext.request.contextPath}/mypage/get_party_board.do");
...
<body onload="partyBoard()">
<jsp:include page="../commons/top_image.jsp"/>

<%--사이드바 시 작 --%>	
  	    <div style="margin-top:50px;position: absolute;">
        <div class="mypage_name"><h2>개설한 파티</h2></div>
			...
			<div class="col">
				<div class="row" style="padding:10px 0px;margin-left:30px;">
					<div class="col-1" style="width:110px;">
						<a href="${pageContext.request.contextPath}/mypage/recruitment_mypage.do" style="text-decoration:none;color:#7DAB12;font-size:15px;font-weight:bold;">개설한 파티 </a>
